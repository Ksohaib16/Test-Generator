...
import { useTest, useUpdateTest, useQuestions } from '@/hooks/use-test-data';
...
export default function EditTest() {
  ...
  const updateTestMutation = useUpdateTest(id);
  ...
  // Update form and selected questions when test data is loaded
  useEffect(() => {
    if (data?.test) {
      ...
      // Set initial filters based on test subject
      setFilters(prev => ({
        ...prev,
        subject: test.subject,
        chapter: test.chapter || '',
  ...
  // Handle errors loading test data
  useEffect(() => {
    if (error) {
      toast({
        title: 'Error',
  ...
  const onSubmit = async (data: TestFormValues) => {
    ...
    try {
      ...
      await updateTestMutation.mutateAsync(testData);
          ...
          <div className="space-y-6 order-1 lg:order-2">
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Update the details for your test
                </CardDescription>
              </CardHeader>
              <CardContent>
                <Form {...form}>
              ...
              <CardFooter className="flex justify-end space-x-2">
                <Button variant="ghost" onClick={() => navigate('/test-library')}>
                  Cancel
                </Button>
                <Button 
                  type="submit" 
                  form="test-form" 
                  disabled={selectedQuestions.length === 0 || updateTestMutation.isPending}
                >
